...
namespace I2C_BMP280
{
  ...
  public class BMP280
  {
    ...
    private readonly BMP280Address m_Address;
    ...
    public BMP280(I2C i2c, BMP280Address address)
    {
      m_I2C = i2c;
      m_Address = address;
      ...
      if (ReadByte(BMP280Register.CHIPID) != 0x58) // check if sensor, i.e. the chip ID is correct
      {
        throw new InvalidOperationException("Device is not a BMP280.");
      }

      // reset the device using soft-reset
      // this makes sure the IIR is off, etc.
      m_I2C.WriteBytes((byte)m_Address, (byte)BMP280Register.SOFTRESET, 0xB6);
    ...
    public float ReadTemperature_C()
    {
      ...
      int adc_T = Read3BytesSignedBigEndian(BMP280Register.TEMPDATA);
    ...
    public float ReadPressure_hPa()
    {
      ...
      int adc_P = Read3BytesSignedBigEndian(BMP280Register.PRESSUREDATA);
    ...
    private void SetSampling(SensorMode mode = SensorMode.MODE_NORMAL, SensorSampling tempSampling = SensorSampling.SAMPLING_X16, SensorSampling pressSampling = SensorSampling.SAMPLING_X16, SensorSampling humSampling = SensorSampling.SAMPLING_X16, SensorFilter filter = SensorFilter.FILTER_OFF, StandbyDuration duration = StandbyDuration.STANDBY_MS_0_5)
    {
      ...
      m_I2C.WriteBytes((byte)m_Address, (byte)BMP280Register.CONTROL, (byte)SensorMode.MODE_SLEEP);

      // you must make sure to also set REGISTER_CONTROL after setting the
      // CONTROLHUMID register, otherwise the values won't be applied (see
      // DS 5.4.3)

      m_I2C.WriteBytes((byte)m_Address, (byte)BMP280Register.CONTROLHUMID, m_ControlHumidity.Get());
      m_I2C.WriteBytes((byte)m_Address, (byte)BMP280Register.CONFIG, m_Config.Get());
      m_I2C.WriteBytes((byte)m_Address, (byte)BMP280Register.CONTROL, m_ControlMeasurement.Get());
    }
    ...
    private Coefficients ReadCoefficients()
    {
      ...
      {
        T1 = Read2BytesLittleEndian(BMP280Register.T1),
        T2 = Read2BytesSignedLittleEndian(BMP280Register.T2),
        T3 = Read2BytesSignedLittleEndian(BMP280Register.T3),

        P1 = Read2BytesLittleEndian(BMP280Register.P1),
        P2 = Read2BytesSignedLittleEndian(BMP280Register.P2),
        P3 = Read2BytesSignedLittleEndian(BMP280Register.P3),
        P4 = Read2BytesSignedLittleEndian(BMP280Register.P4),
        P5 = Read2BytesSignedLittleEndian(BMP280Register.P5),
        P6 = Read2BytesSignedLittleEndian(BMP280Register.P6),
        P7 = Read2BytesSignedLittleEndian(BMP280Register.P7),
        P8 = Read2BytesSignedLittleEndian(BMP280Register.P8),
        P9 = Read2BytesSignedLittleEndian(BMP280Register.P9),

        H1 = ReadByte(BMP280Register.H1),
        H2 = Read2BytesSignedLittleEndian(BMP280Register.H2),
        H3 = ReadByte(BMP280Register.H3),
        H4 = (short)(((sbyte)ReadByte(BMP280Register.H4) << 4) | (ReadByte(BMP280Register.H4 + 1) & 0xF)),
        H5 = (short)(((sbyte)ReadByte(BMP280Register.H5 + 1) << 4) | (ReadByte(BMP280Register.H5) >> 4)),
        H6 = (sbyte)ReadByte(BMP280Register.H6),
      };
    }

    private bool IsBusy()
    {
      return ReadByte(BMP280Register.STATUS).GetBit(0);
    }

    private ushort Read2BytesLittleEndian(BMP280Register register)
    {
    ...
    private short Read2BytesSignedLittleEndian(BMP280Register register)
    {
      m_I2C.WriteBytes((byte)m_Address, (byte)register);
    ...
    private ushort Read2BytesBigEndian(BMP280Register register)
    {
      m_I2C.WriteBytes((byte)m_Address, (byte)register);
    ...
    private int Read3BytesSignedBigEndian(BMP280Register register)
    {
      m_I2C.WriteBytes((byte)m_Address, (byte)register);
    ...
    private byte ReadByte(BMP280Register register)
    {
      m_I2C.WriteBytes((byte)m_Address, (byte)register);
  ...
  public enum BMP280Address : byte
  {
    Primary = 0x77,
    Secondary = 0x76,
  }

  public enum BMP280Register : byte
  {
    T1 = 0x88,
    T2 = 0x8A,
    T3 = 0x8C,
